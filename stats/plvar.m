...
if strcmp(f_dattype,'INTS') && min(x) > 1000,
    f_dattype = 'REAL';
end;
rand('twister',sum(100*clock));         % initialize random number gen.
...
if ~quiet,
    ...
    fprintf('   Copyright 2007 Aaron Clauset\n');
    ...
    case 'INTS',
        ...
        for B=1:size(bof,1)
            ...
            for xm=1:length(ymins)
                ...
                L    = -Inf*ones(size(vec));
                for k=1:length(vec)
                    L(k) = -vec(k)*sum(log(z)) - n*log(zvec(k) - sum((1:xmin-1).^-vec(k)));
                end
                [Y,I] = max(L);
                % compute KS statistic
